...
parser.add_argument("--homs_to_omit", dest="homs_to_omit", help="Hom probes to avoid picking (results of self-blast)", default="designs/{}.blast.bad_homs".format(GENE_NAME))
parser.add_argument("--potential_pair_pickle", dest="potential_pair_pickle", help="Serialized potential pairs file if you give a new file name it creates the data", default="designs/{}.SNPs.pair_file".format(GENE_NAME))
parser.add_argument("--h1_genome_hits", dest="h1_genome_hits", help="pickle file containing hom genome locations for h1", default="designs/{}_xhyb.h1.locations".format(GENE_NAME))
parser.add_argument("--h2_genome_hits", dest="h2_genome_hits", help="pickle file containing hom genome locations for h2", default="designs/{}_xhyb.h2.locations".format(GENE_NAME))
...
for region_index, (start, stop) in enumerate(region_list):
    ...
    for MIP_index, (H2_hom, H1_hom) in enumerate(exon_mips):
        ...
        out_design.write("{}_{}\t{}-{}\t{}-{}\t{}-{}\t{}\t{}\t{}\t{}\t{}\n".format(GENE_NAME, MIP_index + 1, H2_start, H2_stop, gap_fill_start, gap_fill_stop,
