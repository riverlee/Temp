...
parser.add_argument("--allow_snps_in_homs", dest="allow_snps_in_homs", help="Whether allows SNPs in homs", type=int, default=1)
...
parser.add_argument("--allow_snps_in_homs_maxNum", dest="allow_snps_in_homs_maxNum", help="Maxinum allowed number of SNPs in homs", type=int, default=100)
#parser.add_argument("--force",dest="rewrite_pair_file",action="store_true")
#parser.add_argument("--no-force",dest="rewrite_pair_file",action="store_false")
#parser.add_argument("--start_position",dest="start_position",help="Start position to design MIP",type=int)
#parser.add_argument("--stop_position",dest="stop_position", help="End position to design MIP", type=int)
parser.add_argument("--exon_num",dest="exon_num",help="Only design MIP on the specified exon",type=int,required=True)

args = parser.parse_args()

input_bed, input_design= args.input_bed, args.input_design
...
#start_position,stop_position=args.start_position,args.stop_position
...
design_source = design_source[design_source.region_index == exon_num]
...
if make_pairs:
    ...
    #hom_fail_log.write("""hom_name\tH1_H2_both\tseq\tstart\tstop\tgc_pct\trm_pct\thomopolymer_run\tdust_pct_h1\tdust_pct_h2\tgenome_hits_h1\tgenome_hits_h2\tinvalidGC\tinvalidRM\tinvalidDustH1\tinvalidDustH2\tinvalidHomHitH1\tinvalidHomHitH2\thasSNPs\thasSMs\tSelf_Blast_Fail\tH2SNPFail\tH1SNPFail\tH2SNPMinDist\tH1MinDist\tNumerOfSNPs\tSNPs\n""")
    ...
    for probe in design_source.iterrows():
        ...
        ## Check SNPs 1) whether there are snps located to the 'end' of homs; 2) how many SNPs within homs
        # H2 eqauls to upstream and H1 equals to downstream
        region_index = int(probe[1]['region_index'])
        upstream_snp_minD,upstream_snp_num = fetch_snps(region_index,hom_name,start,stop,0)
        downstream_snp_minD,downstream_snp_num = fetch_snps(region_index,hom_name,start,stop,1)

        H2_SNPFail = False
        H1_SNPFail = False
        
        if upstream_snp_minD <= allow_snps_in_homs_distance or upstream_snp_num >= allow_snps_in_homs_maxNum:
            H2_SNPFail = True
        if downstream_snp_minD <= allow_snps_in_homs_distance or downstream_snp_num >= allow_snps_in_homs_maxNum:
            H1_SNPFail = True
        
        if allow_snps_in_homs:
            if H1_SNPFail or rmFail or blastFail or gcFail or hpFail or H1dustFail or H1hitFail or SMFail: validH1 = False
            if H2_SNPFail or rmFail or blastFail or gcFail or hpFail or H2dustFail or H2hitFail or SMFail: validH2 = False
        else:
        ...
        #if failType != "":
            #hom_fail_log.write("""{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\t{13}\t{14}\t{15}\t{16}\t{17}\t{18}\t{19}\t{20}\t{21}\t{22}\t{23}\t{24}\t{25}\t{26}\n""".format(hom_name,failType,hom_seq,start,stop,hom_gc,rm_pct,hom_hp,
            #    hom_dust_H1,hom_dust_H2,hom_hit_H1,hom_hit_H2,gcFail,rmFail,H1dustFail,H2dustFail,H1hitFail,H2hitFail,SNPFail,SMFail,blastFail,H2_SNPFail,H1_SNPFail,upstream_snp_minD,downstream_snp_minD,upstream_snp_num,hom_SNPS))
    ...
    for r_index, (start, stop) in enumerate(region_list):
        ...
        for H2_hom in design_source[(design_source.region_index == r_index) & (design_source.index.isin(valid_H2_indexes))].iterrows():
            ...
            # Find the H1 sorted by ideal_gap_length,which mean find 100gap h1 first, then 99, 101, 98, 102 etc

            H1_tmp = design_source[(design_source.hom_start > H2_stop + gl_min) & (design_source.hom_start <= H2_stop + gl_max) & (design_source.region_index == r_index) & (design_source.index.isin(valid_H1_indexes))]
            if len(list(H1_tmp.iterrows())) == 0: continue

            diff = list(abs(H1_tmp.hom_start-(H2_stop+ideal_gap_length)))
            order = sorted(range(len(diff)),key=lambda x:diff[x])
            order = H1_tmp.index[order]

            H1_tmp_sorted = H1_tmp.reindex(order)
            for H1_hom in H1_tmp_sorted.iterrows():
...
for region_index, (start, stop) in enumerate(region_list):
    ## Required region overlap with exon and do the following
    if region_index != exon_num: continue

    valid_mips = Final_MIP_Set[str(region_index)]
    #last_H2_end, last_H1_start = start - cover_exon_upstream, stop + cover_exon_upstream
    # Fix the first start hom of each exon, the fist hom is far way from cover_exon_upstream (65bp)
    # Rather than cover_exon_upstream - overlap_step (65-50=15bp)
    last_H2_end, last_H1_start = start - cover_exon_upstream-overlap_step, stop + cover_exon_upstream
